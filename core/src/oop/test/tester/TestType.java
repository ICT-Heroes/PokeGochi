...
public class TestType {
	...
	private static boolean isAllTypeIneffective() {
		for (int i = 1; i <= 18; i++) {
			Type t = new Type(i);
			if (t.hasIneffective()) {
				ArrayList<Type> a = t.getIneffective();
			}
		}
		return true;
	}
	...
	private static boolean isAllTypeNoEffect() {
		for (int i = 1; i <= 18; i++) {
			Type t = new Type(i);
			if (t.hasNoEffect()) {
				ArrayList<Type> a = t.getNoEffect();
			}
		}
		return true;
	}
	...
	private static boolean isAllTypeResistance() {
		for (int i = 1; i <= 18; i++) {
			Type t = new Type(i);
			if (t.hasResistance()) {
				ArrayList<Type> a = t.getResistance();
			}
		}
		return true;
	}
	...
	private static boolean isAllTypeSuperEffective() {
		for (int i = 1; i <= 18; i++) {
			Type t = new Type(i);
			if (t.hasSuperEffective()) {
				ArrayList<Type> a = t.getSuperEffective();
			}
		}
		return true;
	}
	...
	private static boolean isAllTypeWeakness() {
		for (int i = 1; i <= 18; i++) {
			Type t = new Type(i);
			if (t.hasWeakness()) {
				ArrayList<Type> a = t.getWeakness();
			}
		}
		return true;
	}
	...
	private static boolean allTypeCheck() {
		if (isAllTypeName() && isAllTypeIneffective() && isAllTypeNoEffect() && isAllTypeResistance()
				&& isAllTypeSuperEffective() && isAllTypeWeakness()) {
			return true;
		} else {
			return false;
		}
	...
	private static boolean typeCheck(int ID) {
		if (isTypeName(ID) && isTypeIneffective(ID) && isTypeNoEffect(ID) && isTypeResistance(ID)
				&& isTypeSuperEffective(ID) && isTypeWeakness(ID)) {
			return true;
		} else {
			return false;
		}
